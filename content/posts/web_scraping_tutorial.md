...
```
Expect to get a sucessful status code in return.
...
See [https://httpstatuses.com/](https://httpstatuses.com/) for a comprehensive list of HTTP status codes.
...
See [interactivepython.org/](https://interactivepython.org/runestone/static/pythonds/Trees/ExamplesofTrees.html)
...
Now let's loop over the returned links and build a list of topics using a list comprehension.
```python
topics = [topic for topic in soup.select('li > a')]
topics[:8]
```
...
Expect to see sucess, if not, double check the link you specify.
...
1. Other Practice Sites
    ...
    * [Books to Scrape Site](http://books.toscrape.com/)

    * [Quotes to Scrape Site](http://quotes.toscrape.com/)
